  ...
  return (
    <div className={cn("space-y-2", fullscreen && "fixed inset-3 z-[70] flex flex-col rounded-3xl bg-white p-3 shadow-2xl ring-1 ring-black/10", className)}>
      ...
      <div className="overflow-hidden rounded-3xl border border-dark-200 bg-white shadow-[0_14px_50px_rgba(15,23,42,.08)]">
        ...
        {sourceMode ? <textarea value={value} onChange={(event) => onChange(sanitizeRichHtml(event.target.value))} aria-label="HTML sumber" spellCheck={false} className="block w-full resize-none border-0 bg-[#0b1220] p-5 font-mono text-xs leading-6 text-slate-100 outline-none" style={{ minHeight }} /> : <div ref={editorRef} contentEditable suppressContentEditableWarning role="textbox" aria-multiline="true" aria-label={label || "Editor konten"} data-placeholder={placeholder} onInput={commit} onBlur={saveSelection} onKeyDown={onKeyDown} onKeyUp={saveSelection} onMouseUp={saveSelection} onFocus={saveSelection} onPaste={onPaste} className="rich-editor-content max-w-none overflow-auto px-6 py-5 text-sm leading-7 text-dark outline-none" style={{ minHeight }} />}
        <div className="flex items-center justify-between border-t border-dark-100 bg-dark-50/50 px-4 py-2 text-[11px] text-dark-400"><span>⌘/Ctrl+B · I · U · K</span><span>{wordCount.toLocaleString("id-ID")} kata</span></div>
      </div>
    </div>
  );
}
