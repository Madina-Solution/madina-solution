...
export default async function BlogDetailPage({ params }: Props) {
  ...
  const [locale, t] = await Promise.all([getLocale(), getTranslations("BlogArticle")]);
  ...
  return (
    <>
      ...
      <ArticleSchema name={article.title} description={article.excerpt || `Artikel ${article.title}`} url={pageUrl} image={article.thumbnail || undefined} publishedAt={article.publishedAt?.toISOString()} updatedAt={updatedDate?.toISOString()} authorName={editorialName} inLanguage={locale === "en" ? "en-US" : "id-ID"} />
            ...
            <aside className="min-w-0 lg:sticky lg:top-24">
              ...
              <div className="mt-5 rounded-3xl border border-dark-100 bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 space-y-2 text-sm text-dark-600">
                  ...
                  <div className="flex justify-between gap-4"><span>{t("updated")}</span><strong className="text-right text-dark">{formatDate(updatedDate || article.createdAt)}</strong></div>
