...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = z.object({ name: z.string().min(2), role: z.string().optional(), company: z.string().optional(), content: z.string().min(5), rating: z.number().int().min(1).max(5).optional(), isFeatured: z.boolean().optional() }).safeParse(body);
