...
import { desc, eq, ilike, or, count, isNull } from "drizzle-orm";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [totalResult] = await db.select({ value: count() }).from(media).where(conditions.length > 0 ? conditions.reduce((a, b) => a!) : undefined);
