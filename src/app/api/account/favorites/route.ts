...
export async function GET() {
  try {
    ...
    const list = await db.select({ id: favorites.id, productId: favorites.productId, createdAt: favorites.createdAt, productName: products.name, productSlug: products.slug, productPrice: products.basePrice, productUnit: products.unit }).from(favorites).leftJoin(products, eq(favorites.productId, products.id)).where(eq(favorites.userId, session.userId)).orderBy(desc(favorites.createdAt));
