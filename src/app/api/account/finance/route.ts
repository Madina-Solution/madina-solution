  ...
  try {
    ...
    const [customerOrders, customerPayments, allCustomerOrders, allCustomerPayments] = await Promise.all([
      ...
      db.select({ id: orders.id, total: orders.total, status: orders.status })
        .from(orders).where(eq(orders.userId, session.userId)),
      db.select({ orderId: payments.orderId, amount: payments.amount, status: payments.status })
        .from(payments).innerJoin(orders, eq(payments.orderId, orders.id)).where(eq(orders.userId, session.userId)),
    ]);

    const activeOrderTotals = new Map(allCustomerOrders.filter((order) => order.status !== "cancelled").map((order) => [order.id, Number(order.total || 0)]));
    ...
    const pendingVerification = allCustomerPayments.filter((payment) => payment.status === "pending_verification").reduce((sum, payment) => sum + Number(payment.amount || 0), 0);
