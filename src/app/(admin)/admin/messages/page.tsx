...
type ThreadResponse = { customer: CustomerContext | null; customerOrders: CustomerOrder[]; financialSummary: { paidTotal: number; outstanding: number }; messages: ThreadMsg[] };
...
export default function AdminMessagesPage() {
  ...
  const fetchThread = React.useCallback(async (customerId: string, silent = false) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setContext({ customer: data.customer, customerOrders: data.customerOrders || [], financialSummary: data.financialSummary || { paidTotal: 0, outstanding: 0 }, messages: data.messages || [] });
    } catch {
      ...
      <div className="grid overflow-hidden rounded-[26px] border border-dark-100 bg-white shadow-[0_18px_55px_rgba(15,23,42,.06)] dark:border-slate-800 dark:bg-slate-950 lg:grid-cols-[320px_minmax(0,1fr)_290px]" style={{ minHeight: "calc(100vh - 265px)" }}>
        ...
        <aside className="hidden border-l border-dark-100 bg-dark-50/60 p-4 dark:border-slate-800 dark:bg-slate-900/50 lg:block">
          {!selected || !customer ? <div className="flex h-full items-center justify-center text-center"><div><UserCircle className="mx-auto h-9 w-9 text-dark-300 dark:text-slate-700" /><p className="mt-3 text-sm font-bold text-dark dark:text-white">Konteks pelanggan</p><p className="mt-1 text-xs leading-5 text-dark-500 dark:text-slate-400">Profil dan ringkasan finansial muncul setelah percakapan dipilih.</p></div></div> : <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2"><div className="rounded-2xl border border-dark-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950"><p className="text-[10px] font-bold uppercase text-dark-400">Sudah dibayar</p><p className="mt-1 text-sm font-black text-green-600">Rp {Math.round(finance?.paidTotal || 0).toLocaleString("id-ID")}</p></div><div className="rounded-2xl border border-dark-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950"><p className="text-[10px] font-bold uppercase text-dark-400">Piutang</p><p className="mt-1 text-sm font-black text-amber-600">Rp {Math.round(finance?.outstanding || 0).toLocaleString("id-ID")}</p></div></div>
            ...
            <div className="rounded-2xl border border-blue-200 bg-blue-50 p-3.5 text-xs leading-5 text-blue-800 dark:border-blue-900/70 dark:bg-blue-950/20 dark:text-blue-200"><Banknote className="mb-1 h-4 w-4" />Gunakan konteks keuangan ini sebelum memberi informasi tentang pembayaran. Angka berasal dari transaksi yang sudah tercatat, bukan asumsi chat.</div>
          </div>}
        </aside>
      </div>
