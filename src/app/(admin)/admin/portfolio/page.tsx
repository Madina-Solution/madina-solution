...
import { Plus, Pencil, Trash2, Loader2, Image, Star } from "lucide-react";
...
type PortfolioItem = { id: string; title: string; slug: string; description: string | null; category: string | null; client: string | null; isFeatured: boolean | null; isActive: boolean };
...
export default function AdminPortfolioPage() {
  ...
  const [form, setForm] = React.useState({ title: "", slug: "", description: "", category: "", client: "", isFeatured: false, isActive: true });
  ...
  const resetForm = () => { setShowForm(false); setEditId(null); setForm({ title: "", slug: "", description: "", category: "", client: "", isFeatured: false, isActive: true }); };
  ...
  const startEdit = (p: PortfolioItem) => { setForm({ title: p.title, slug: p.slug, description: p.description || "", category: p.category || "", client: p.client || "", isFeatured: !!p.isFeatured, isActive: p.isActive }); setEditId(p.id); setShowForm(true); };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-dark-100 bg-white">
        ...
        : items.length > 0 ? (
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-dark-100 text-left">
            ...
            <tr key={p.id} className="border-b border-dark-50 last:border-0 hover:bg-dark-50/50">
              <td className="px-5 py-3.5 font-medium text-dark">{p.title}</td><td className="px-5 py-3.5 text-dark-600">{p.category || "—"}</td><td className="px-5 py-3.5 text-dark-600">{p.client || "—"}</td>
        ...
        ) : <div className="flex flex-col items-center justify-center py-16"><Image className="h-10 w-10 text-dark-300" aria-hidden="true" /><p className="mt-4 font-semibold text-dark">Belum ada portfolio</p></div>}
      </div>
