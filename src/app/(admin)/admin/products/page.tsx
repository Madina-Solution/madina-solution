...
export default function AdminProductsPage() {
  ...
  const [form, setForm] = React.useState({
    ...
    basePrice: "", unit: "pcs", minOrder: 1, productionDays: 3, isFeatured: false, isActive: true,
  });
  ...
  const startEdit = async (id: string) => {
    ...
    if (data.success) {
      ...
      setForm({ name: p.name, slug: p.slug, categoryId: p.categoryId || "", shortDescription: p.shortDescription || "", description: p.description || "", basePrice: p.basePrice, unit: p.unit || "pcs", minOrder: p.minOrder || 1, productionDays: p.productionDays || 3, isFeatured: p.isFeatured || false, isActive: p.isActive });
  ...
  const resetForm = () => {
    ...
    setForm({ name: "", slug: "", categoryId: "", shortDescription: "", description: "", basePrice: "", unit: "pcs", minOrder: 1, productionDays: 3, isFeatured: false, isActive: true });
  };
          ...
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-dark-100 text-left">
            ...
            {filteredProducts.map((p) => (
              <tr key={p.id} className="border-b border-dark-50 transition-colors hover:bg-dark-50/50 last:border-0">
                <td className="px-5 py-3.5"><p className="font-medium text-dark">{p.name}</p><p className="text-xs text-dark-400">/{p.slug}</p></td>
