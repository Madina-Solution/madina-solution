...
export default function AdminCategoriesPage() {
  ...
  const [formData, setFormData] = React.useState({ name: "", slug: "", description: "", order: 0 });
  ...
  const handleEdit = (cat: Category) => {
    setEditId(cat.id);
    setFormData({ name: cat.name, slug: cat.slug, description: cat.description || "", order: cat.order || 0 });
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setShowForm(true); setEditId(null); setFormData({ name: "", slug: "", description: "", order: 0 }); }}>
              ...
              <tbody>
                {categories.map((cat) => (
                  <tr key={cat.id} className="border-b border-dark-50 last:border-0">
                    <td className="px-6 py-3.5 font-medium text-dark">{cat.name}</td>
