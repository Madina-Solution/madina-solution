...
type FormState = {
  ...
  socialInstagram: string; socialFacebook: string; socialTiktok: string; socialYoutube: string; socialLinkedin: string;
  ...
  const fetchSettings = React.useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        setForm({
          siteName: getString(s, "site_name", INITIAL.siteName), siteTagline: getString(s, "site_tagline", INITIAL.siteTagline), siteLogo: getString(s, "site_logo", INITIAL.siteLogo), siteIcon: getString(s, "site_icon", INITIAL.siteIcon), siteEmail: getString(s, "site_email", INITIAL.siteEmail), sitePhone: getString(s, "site_phone", INITIAL.sitePhone), siteAddress: getString(s, "site_address", INITIAL.siteAddress), siteWhatsapp: getString(s, "site_whatsapp", INITIAL.siteWhatsapp), siteUrl: getString(s, "site_url", window.location.origin), socialInstagram: getString(s, "social_instagram"), socialFacebook: getString(s, "social_facebook"), socialTiktok: getString(s, "social_tiktok"), socialYoutube: getString(s, "social_youtube"), socialLinkedin: getString(s, "social_linkedin"), seoTitle: getString(s, "seo_title"), seoDescription: getString(s, "seo_description", INITIAL.seoDescription), seoKeywords: getString(s, "seo_keywords", INITIAL.seoKeywords), seoOgImage: getString(s, "seo_og_image"), seoTwitterHandle: getString(s, "seo_twitter_handle"),
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <Card><CardContent className="p-6"><h2 className="flex items-center gap-2 font-semibold text-dark"><Globe className="h-5 w-5 text-primary" />Branding & Identitas</h2><p className="mt-1 text-sm text-dark-500">Nama dan logo di bawah akan digunakan pada header, mobile navigation dan metadata situs.</p><div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 rounded-2xl border border-dark-100 bg-dark-50 p-4"><div className="flex items-start gap-3"><div className="rounded-xl bg-white p-2 shadow-sm"><Globe className="h-4 w-4 text-primary" /></div><div><p className="text-sm font-bold text-dark">Media Sosial (opsional)</p><p className="mt-1 text-xs leading-5 text-dark-500">Saat ini website cukup memakai WhatsApp dan Email. Field ini disiapkan untuk Instagram, Facebook, TikTok, YouTube, dan LinkedIn; link yang kosong tidak akan ditampilkan di website.</p></div></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><Input value={form.socialInstagram} onChange={(e) => update("socialInstagram", e.target.value)} placeholder="https://instagram.com/..." aria-label="Instagram" /><Input value={form.socialFacebook} onChange={(e) => update("socialFacebook", e.target.value)} placeholder="https://facebook.com/..." aria-label="Facebook" /><Input value={form.socialTiktok} onChange={(e) => update("socialTiktok", e.target.value)} placeholder="https://tiktok.com/@..." aria-label="TikTok" /><Input value={form.socialYoutube} onChange={(e) => update("socialYoutube", e.target.value)} placeholder="https://youtube.com/@..." aria-label="YouTube" /><Input value={form.socialLinkedin} onChange={(e) => update("socialLinkedin", e.target.value)} placeholder="https://linkedin.com/company/..." aria-label="LinkedIn" /></div></div>
